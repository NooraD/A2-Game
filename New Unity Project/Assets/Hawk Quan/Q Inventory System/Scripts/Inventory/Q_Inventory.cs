...
public class Q_Inventory : MonoBehaviour
{
    ...
    protected virtual void Start()
    {
        ...
        {
            items.Add(Itemm.CreateInstance<Itemm>());
    ...
    {
        Itemm newItem = Q_GameMaster.Instance.inventoryManager.itemDataBase.getItemByID(id);
    ...
    public virtual ItemData AddItem(Itemm newItem,int slotID,int amount)
    {
        Debug.Log("get " + newItem.itemName);
    ...
    public void CDAllThis(Itemm item)
    {
        for (int i = 0; i < items.Count; i++)
        {
            if (items[i] == item)
    ...
    ItemData AddItemAtNewSlot(Itemm newItem,int amountToAdd)
    {
        for (int i = 0; i < items.Count; i++)
        {
            if (items[i].ID == -1)
